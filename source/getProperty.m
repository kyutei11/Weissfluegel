function [spanwise, total] = getProperty(alpha,beta,p,r, phi, d_aile, com)
  ...
  for i=1:2*com.n
    dF=vprd(spanwise.ut(i+1,:) ,dGamma(i+1)*[1 0 0]*norm(com.x(i+1,:)-com.xtv(i+1,:))*3*0.75 );
    spanwise.F_Gamma_tv(i,:) = dF;
    
    # xtv=x-[1 0 0]*c/3;
    total.F_tv = total.F_tv + dF;
    total.M_tv = total.M_tv + vprd(com.xtv(i,:),dF);
